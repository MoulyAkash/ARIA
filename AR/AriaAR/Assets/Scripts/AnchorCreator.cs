...
public class AnchorCreator : MonoBehaviour
{
    ...
    private bool Pos2Anchor(float x, float y, BoundingBox outline)
    {
        ...
        anchorObj_mesh.text = $"{outline.Label}";
        if (Application.platform == RuntimePlatform.Android)
        {
            using (AndroidJavaClass jc = new AndroidJavaClass("com.azesmwayreactnativeunity.ReactNativeUnityViewManager"))
            {
                jc.CallStatic("sendMessageToMobileApp", $"{outline.Label}:{(int)(outline.Confidence * 100)}%");
            }
        }
        // anchorObj_mesh.GetComponent<TextMeshProUGUI>().text = $"{outline.Label}: {(int)(outline.Confidence * 100)}%";
        // Perform the raycast
